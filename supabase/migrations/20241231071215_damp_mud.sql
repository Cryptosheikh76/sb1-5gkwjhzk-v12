/*
  ...
  1. Changes
    - Add new columns to NFT collections
    - Add new columns to NFT items
    - Add new columns to NFT listings and offers
    - Update RLS policies
  2. Security
...
BEGIN
    ...
    DROP POLICY IF EXISTS "nft_collections_select_policy_v5" ON nft_collections;
    CREATE POLICY "nft_collections_select_policy_v5"
        ON nft_collections FOR SELECT
        USING (true);

    DROP POLICY IF EXISTS "nft_collections_all_policy_v5" ON nft_collections;
    CREATE POLICY "nft_collections_all_policy_v5"
        ON nft_collections FOR ALL
        USING (creator_id = auth.uid());

    -- Items policies
    DROP POLICY IF EXISTS "nft_items_select_policy_v5" ON nft_items;
    CREATE POLICY "nft_items_select_policy_v5"
        ON nft_items FOR SELECT
        USING (true);

    DROP POLICY IF EXISTS "nft_items_all_policy_v5" ON nft_items;
    CREATE POLICY "nft_items_all_policy_v5"
        ON nft_items FOR ALL
        USING (owner_id = auth.uid());

    -- Listings policies
    DROP POLICY IF EXISTS "nft_listings_select_policy_v5" ON nft_listings;
    CREATE POLICY "nft_listings_select_policy_v5"
        ON nft_listings FOR SELECT
        USING (true);

    DROP POLICY IF EXISTS "nft_listings_all_policy_v5" ON nft_listings;
    CREATE POLICY "nft_listings_all_policy_v5"
        ON nft_listings FOR ALL
        USING (seller_id = auth.uid());

    -- Offers policies
    DROP POLICY IF EXISTS "nft_offers_select_policy_v5" ON nft_offers;
    CREATE POLICY "nft_offers_select_policy_v5"
    ...
    DROP POLICY IF EXISTS "nft_offers_insert_policy_v5" ON nft_offers;
    CREATE POLICY "nft_offers_insert_policy_v5"
    ...
    DROP POLICY IF EXISTS "nft_offers_update_policy_v5" ON nft_offers;
    CREATE POLICY "nft_offers_update_policy_v5"
